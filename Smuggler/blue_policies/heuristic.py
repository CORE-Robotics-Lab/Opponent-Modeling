...
class BlueHeuristic:
    ...
    def predict(self, blue_observation: np.ndarray):
        ...
        if np.array_equiv(new_detection, np.array([-1, -1])):
            ...
            new_detection = [new_detection[0] * self.env.dim_x , new_detection[1] * self.env.dim_y]
    ...
    def debug_plot_plans(self):
        self.fig, self.ax = plt.subplots()
        self.ax.set_xlim([0, int(self.env.dim_x)])
        self.ax.set_ylim([0, int(self.env.dim_y)])
        self.ax.set_aspect('equal')

        all_blue = self.env.search_parties_list + self.env.helicopters_list if self.env.is_helicopter_operating() else self.env.search_parties_list
